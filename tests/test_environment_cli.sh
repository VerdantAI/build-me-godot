...
jq -e '.environment.checks | any(.id == "conformance.provider.trellis" and .status == "warning" and .importance == "optional")' <<<"$conformance_plan_output" >/dev/null
...
jq -e '.environment.checks[] | select(.id == "conformance.provider.trellis") | .evidence.provider_status == "experimental_user_managed_command" and .evidence.command_contract.arguments == ["--input", "<image>", "--output", "<mesh.glb>", "--metadata-output", "<metadata.json>"] and .evidence.command_contract.automatic_downloads_allowed == false' <<<"$conformance_plan_output" >/dev/null
...
jq -e '.install_plan.actions[] | select(.id == "install.manual.trellis.provider") | .mode == "manual" and .license.code == "MIT" and .license.weights == "MIT" and .license.commercial_use == true and .command_contract.automatic_downloads_allowed == false' <<<"$conformance_plan_output" >/dev/null
